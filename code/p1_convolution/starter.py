...
def conv2d_loops(img, K):
    ...
    for y in range(h):
        for x in range(w):
            ...
            for ky in range(kh):
                for kx in range(kw):
                    val += padded[y + ky, x + kx] * K[kh - 1 - ky, kw - 1 - kx]
            out[y, x] = val
    return out
...
def conv2d_taps(img, K):
    ...
    for ky in range(kh):
        for kx in range(kw):
            out += padded[ky:ky+h, kx:kx+w] * K[kh - 1 - ky, kw - 1 - kx]
    return out
...
def conv2d_im2col(img, K):
    ...
    patches = np.lib.stride_tricks.sliding_window_view(padded, (kh, kw))
    patches_flat = patches.reshape(h * w, kh * kw)
...
def conv2d_fft(img, K):
    ...
    img_f = np.fft.fft2(img, s=(out_h, out_w))
    K_f = np.fft.fft2(K, s=(out_h, out_w))
